  ...
  return <div className="page-canvas section-page memory-page">
    ...
    {data && <>
      <section className="graph-summary category-summary">
        ...
        <form className="identity-filter" onSubmit={applyIdentity}><label>Identity<input aria-label="Filter category atlas by identity" list="memory-identities" onChange={event => setIdentityDraft(event.target.value)} placeholder="user_id or logical agent" ref={identityInput} value={identityDraft} /><datalist id="memory-identities">{data.identities.map(item => <option key={`${item.kind}:${item.identity}`} value={`${item.kind}:${item.identity}`} />)}</datalist></label><button type="submit">Apply</button>{identity !== "all" && <button onClick={() => { setIdentity("all"); setIdentityDraft(""); resetAtlas(); }} type="button">Clear</button>}</form>
      </section>
