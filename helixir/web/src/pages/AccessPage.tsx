...
export function AccessPage({ initialTab = "groups", initialOnlineOnly = false }: { initialTab?: AccessTab; initialOnlineOnly?: boolean }) {
  ...
  const active = data?.agents.filter(agent => agent.active).length ?? 0;
  const filteredAgents = useMemo(() => data?.agents.filter(agent => (!onlineOnly || agent.active) && searchable(agent).includes(query.toLowerCase())) ?? [], [data, query, onlineOnly]);
  ...
  return <div className="page-canvas section-page access-page">
    ...
    <section className="mini-metrics" aria-label="Access registry views"><button aria-pressed={tab === "agents"} className={tab === "agents" ? "is-active" : ""} onClick={() => { setTab("agents"); setOnlineOnly(false); setQuery(""); setExactPrincipal(null); setPage(1); }}><span>Agents</span><strong>{data?.agents.length ?? "—"}</strong><small>{active} online now · open roster</small><i>↗</i></button><button aria-pressed={tab === "principals"} className={tab === "principals" ? "is-active" : ""} onClick={() => { setTab("principals"); setOnlineOnly(false); setQuery(""); setExactPrincipal(null); setPage(1); }}><span>Principals</span><strong>{data?.principals.length ?? "—"}</strong><small>RBAC identities · inspect roles</small><i>↗</i></button><button aria-pressed={tab === "groups"} className={tab === "groups" ? "is-active" : ""} onClick={() => { setTab("groups"); setOnlineOnly(false); setQuery(""); setExactPrincipal(null); setPage(1); }}><span>Groups</span><strong>{data?.groups.length ?? "—"}</strong><small>security domains · administer</small><i>↗</i></button></section>
...
function AgentRegistry({ data, agents, onChanged }: { data: AccessProjection; agents: AccessProjection["agents"]; onChanged: () => void }) {
  return <div className="access-accordion">{agents.map(agent => {
    const principal = data.principals.find(item => item.subject_id === agent.agent_id);
    return <details key={agent.agent_id}><summary><span className="identity-mark">{agent.agent_id.slice(0, 2).toUpperCase()}</span><div><strong>{agent.name || agent.agent_id}</strong><small>{agent.agent_id} · {agent.host || "unknown host"}</small></div><div className={agent.active ? "agent-summary-state is-online" : "agent-summary-state"}><StatusDot ok={agent.active} pulse={agent.active} /><b>{agent.active ? "ONLINE" : relativeAge(agent.age_seconds)}</b></div><i>＋</i></summary><div className="agent-detail"><dl><div><dt>Runtime role</dt><dd>{agent.role || "agent"}</dd></div><div><dt>Host</dt><dd>{agent.host || "unknown"}</dd></div><div><dt>Last signal</dt><dd>{agent.last_seen || "unknown"}</dd></div><div><dt>Reported status</dt><dd>{agent.status || "silent"}</dd></div></dl><div><h3>RBAC memberships</h3><div className="role-stack is-left">{principal?.global_roles.map(role => <em className="role-chip is-global" key={role}>{role}</em>)}{principal?.groups.flatMap(group => group.roles.map(role => <em className="role-chip" key={`${group.group_id}-${role}`}>{group.group_id} / {role}</em>))}{!principal && <span className="empty-row">No graph-backed principal binding.</span>}</div>{!agent.active && <DangerAction label="Prune junk presence" body={{ id: agent.agent_id }} path="/access/agents/prune" confirm={`Permanently prune '${agent.agent_id}' and its presence provenance? Use this only for test or renamed identities; stale agents need no pruning.`} onChanged={onChanged} />}</div></div></details>;
  })}</div>;
}
