...
//!
//! The NLI runtime is part of every build. If the model files are unexpectedly
//! missing or unloadable, the router fails open to the existing LLM relation
//! inference path; onboarding and `helixir doctor` prevent that degraded state
//! from being reported as a ready installation.
        ...
        for (cand_id, cand_content) in candidates {
            ...
            let (rel_type, prob) = match verdict {
                ...
                Ok((crate::llm::nli::NliLabel::Contradiction, p))
                    if p[0] >= min_prob
                        && crate::core::charter::shares_subject(&new_content, &cand_content) =>
                {
