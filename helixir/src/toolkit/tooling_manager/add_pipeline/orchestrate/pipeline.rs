//! Core add-memory decision and persistence pipeline.
...
impl ToolingManager {
    ...
    pub(super) async fn run_add_pipeline(
        &self,
        ...
        let mut clarifications: Vec<super::super::super::types::Clarification> = Vec::new();
        ...
        for (i, memory) in memories_to_store.iter().enumerate() {
            ...
            let candidate_ids = similar_results
                ...
                .collect::<Vec<_>>();
            let candidate_refs = similar_results
                .iter()
                .filter_map(|result| {
                    result
                        .internal_id
            ...
            let allowed = if candidate_refs.len() == similar_results.len() {
        ...
        let (relations_created, chunks_created) = self
            .finalize_relations_and_raw(
                infer_jobs,
                extracted_relations,
                &memories_to_store,
                &stored_memory_ids,
