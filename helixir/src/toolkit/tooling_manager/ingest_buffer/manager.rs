//! Queue administration methods on the tooling manager.
...
impl ToolingManager {
    ...
    pub(super) async fn fetch_pending_batch(
        &self,
        limit: i64,
    ) -> Result<Vec<PendingNode>, ToolingError> {
    ...
    pub(super) async fn recover_stuck_processing(&self) {
        let params = serde_json::json!({
    ...
    pub(super) async fn process_one_pending(&self, node: &PendingNode) -> bool {
        ...
        let outcome = loop {
            attempt += 1;
            let authorized = match rbac
                .authorize_and_resolve_write_scope(&node.actor_id, &node.user_id, group)
                .await
            {
