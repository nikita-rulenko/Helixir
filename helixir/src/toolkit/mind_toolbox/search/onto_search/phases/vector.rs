...
use super::super::temporal::{calculate_temporal_freshness, is_within_temporal_window};
...
pub async fn vector_search_phase(
    ...
    let result: VectorResult = match client
    ...
    for mem in all_memories {
        let Some(memory_id) = mem.get("memory_id").and_then(|v| v.as_str()) else {
            continue;
        };
        ...
        if !is_within_temporal_window(created_at, config.temporal_hours) {
            continue;
        }
