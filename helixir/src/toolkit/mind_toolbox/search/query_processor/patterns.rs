...
lazy_static! {
    pub static ref EXPANSION_MAPPINGS: HashMap<&'static str, Vec<&'static str>> = {
        ...
        m.insert(
            "like",
            vec!["love", "enjoy", "prefer", "fond of", "appreciate"],
        );
        m.insert("love", vec!["like", "adore", "enjoy", "passionate about"]);
        m.insert("prefer", vec!["like", "favor", "choose", "opt for"]);
        m.insert(
            "can",
            vec!["able to", "capable of", "know how to", "proficient in"],
        );
        m.insert(
            "skill",
            vec!["ability", "expertise", "competence", "proficiency"],
        );
        m.insert("want", vec!["wish", "desire", "aim", "plan", "intend"]);
        m.insert(
            "goal",
            vec!["objective", "target", "aim", "ambition", "plan"],
        );
        m.insert(
            "python",
            vec!["programming", "coding", "development", "backend"],
        );
        m.insert(
            "ai",
            vec!["artificial intelligence", "machine learning", "ml", "llm"],
        );
