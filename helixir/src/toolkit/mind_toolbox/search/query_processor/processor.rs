...
use super::patterns::{EXPANSION_MAPPINGS, detect_intent, intent_to_concept};
...
impl QueryProcessor {
    ...
    pub fn process(&self, query: &str) -> ProcessedQuery {
        ...
        let detected_intents: Vec<String> = detected_intents_raw
            .iter()
            .map(|&s| s.to_string())
            .collect();
        ...
        info!(
            "Query processed with {} intents, confidence: {}",
