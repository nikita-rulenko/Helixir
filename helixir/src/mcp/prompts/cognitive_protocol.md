...
- New onboarding enables RBAC through the `onboarding` enrollment/compatibility group. Existing users migrate as workers, selected clients as group admins, and only the operator receives global admin. `--legacy-trusted-mode` is the explicit RBAC-disabled escape hatch. Enabled authorization is deny-by-default and fail-closed.
- Active or historical onboarding membership defines the graph-backed principal registry. Administrators enroll users in `onboarding` before assigning other groups; removal preserves the User node and role history. Never create a second local registry.
...
- Enrolled onboarding writers may omit `group_id`; Helixir routes them to `onboarding`. Every write to another group must name that concrete `group_id`. Never pass a dedup federation id as `group_id`.
- Compatibility-group writes preserve legacy-global fingerprints so migrated and new memories continue to deduplicate. Do not grant every agent global admin: only the bootstrap operator owns the RBAC control plane.
