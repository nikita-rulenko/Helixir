    ...
    async fn search_memory(
            ...
            && self.client().config().mode.collective_enabled()
        {
            contents.push(Content::text(format!(
                "Hint: personal scope returned {} result(s). If you expected more, retry search_memory with scope=\"collective\" inside your RBAC visibility. A global admin may use list_users(actor_id=...) to inspect the registry; other agents should verify their configured owner id.",
                results.len()
            )));
        }
