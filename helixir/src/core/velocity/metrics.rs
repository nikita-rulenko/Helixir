...
mod tests {
    ...
    fn test_velocity_score_max() {
        let score = calculate_velocity_score(3600.0, 10.0, 10, 5);
        assert!((score - 100.0).abs() < 0.01);
    }

    #[test]
    fn test_velocity_score_zero() {
        let score = calculate_velocity_score(0.0, 0.0, 0, 0);
    ...
    fn test_velocity_score_slow_resolution() {
        let score = calculate_velocity_score(168.0 * 3600.0, 5.0, 5, 3);
