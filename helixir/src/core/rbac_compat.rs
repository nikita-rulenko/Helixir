...
/// Reserved system workspace for hypotheses and other memory generated by the Moirai.
pub const MOIRAI_GROUP_ID: &str = "moirai";
...
const MOIRAI_GROUP_NAME: &str = "Moirai";
const MOIRAI_GROUP_DESCRIPTION: &str =
    "Global-admin-only workspace for generative memory and hypotheses";
...
pub struct CompatibilityBootstrapReport {
    ...
    pub moirai_memories_repaired: usize,
}
...
impl RbacManager {
    ...
    /// Converge a fresh or legacy store on the permanent workspace model.
    ...
    pub async fn bootstrap_compatibility(
        ...
        if initial.migration_state == RbacMigrationState::Active {
            ...
            if !initial.groups.contains_key(MOIRAI_GROUP_ID) {
                self.create_group_as(
                    MOIRAI_GROUP_ID,
                    MOIRAI_GROUP_NAME,
                    MOIRAI_GROUP_DESCRIPTION,
                    operator_id,
                )
                .await?;
            }
            let current = self.snapshot().await?;
            if !current.groups.contains_key(DEFAULT_GROUP_ID)
                || !current.groups.contains_key(ONBOARDING_GROUP_ID)
                || !current.groups.contains_key(MOIRAI_GROUP_ID)
            {
                bail!("active RBAC migration checkpoint has an incomplete policy graph");
            }
            let mut changed = false;
            for principal in &enrolled {
                if !principal_ready(&current, principal) {
                    self.grant(
                        principal,
                        Role::Worker,
            ...
            let moirai_memories_repaired = self.repair_moirai_memory_layer(operator_id).await?;
        ...
        if !policy.enabled
            ...
            || !policy.groups.contains_key(MOIRAI_GROUP_ID)
        {
