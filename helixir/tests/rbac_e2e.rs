...
async fn rbac_dedup_federation_preserves_history_and_isolates_future_writes() {
    ...
    common::with_cleanup(
        async {
            ...
            let multi_group_id =
                saved_id(&multi_group_result).expect("multi-group saved memory id");
            assert!(
        ...
        async {
            ...
            for (user, role, group) in [
                ...
                if let Err(error) = rbac.revoke_as(user, role, Some(group), &admin).await {
                    errors.push(format!("revoke {user}/{group}: {error}"));
                }
            }
            for user in [
                &worker_a,
                ...
                if let Err(error) = rbac
                    .revoke_as(user, Role::Worker, Some("onboarding"), &admin)
                    .await
                {
                    errors.push(format!("offboard {user}: {error}"));
                }
            }
            for group in [&group_a, &group_b, &group_c] {
                if let Err(error) = rbac.detach_group_from_dedup_as(group, &admin).await {
                    errors.push(format!("detach {group}: {error}"));
                }
                if let Err(error) = rbac.deactivate_group_as(group, &admin).await {
                    errors.push(format!("deactivate {group}: {error}"));
                }
            }
            if let Err(error) = rbac.deactivate_dedup_group_as(&dedup_group, &admin).await {
                errors.push(format!("deactivate dedup group: {error}"));
            }
            errors
        },
    )
