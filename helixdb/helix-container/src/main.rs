...
fn main() {
    ...
    let _heap_profiler = {
        let output =
            std::env::var("HELIX_DHAT_PATH").unwrap_or_else(|_| "dhat-heap.json".to_string());
        dhat::Profiler::builder()
            .file_name(output)
            .trim_backtraces(Some(64))
            .build()
    };
    ...
    let workers_per_core = GatewayOpts::workers_per_core_from_env()
        .unwrap_or_else(|error| panic!("Invalid HelixDB worker configuration: {error}"));
