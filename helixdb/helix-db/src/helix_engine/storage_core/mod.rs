...
impl HelixGraphStorage {
    pub fn new(
        ...
        let force_reindex = std::env::var("HELIX_REINDEX_SECONDARY")
            .map(|value| matches!(value.as_str(), "1" | "true" | "yes"))
            .unwrap_or(false);
        let report = storage.ensure_secondary_indices(force_reindex)?;
        if report.rebuilt_indices > 0 {
            tracing::info!(
                rebuilt_indices = report.rebuilt_indices,
                scanned_nodes = report.scanned_nodes,
                inserted_entries = report.inserted_entries,
