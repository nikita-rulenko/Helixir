...
#[test]
fn startup_backfills_a_new_index_for_existing_nodes() {
    let temp_dir = TempDir::new().unwrap();
    let db_path = temp_dir.path().to_str().unwrap();

    {
        let storage = Arc::new(
            HelixGraphStorage::new(db_path, Default::default(), Default::default()).unwrap(),
        );
        let arena = Bump::new();
        ...
        G::new_mut(&storage, &arena, &mut txn)
            .add_n(
                "person",
                props_option(&arena, props! { "name" => "existing" }),
                None,
            )
            .collect_to_obj()
            .unwrap();
    ...
    config.graph_config.as_mut().unwrap().secondary_indices =
        Some(vec![SecondaryIndex::Index("name".to_string())]);
    let storage = HelixGraphStorage::new(db_path, config.clone(), Default::default()).unwrap();
    let arena = Bump::new();
    let txn = storage.graph_env.read_txn().unwrap();
    let nodes = G::new(&storage, &txn, &arena)
        .n_from_index("person", "name", &"existing".to_string())
    ...
    assert_eq!(
        nodes.len(),
        1,
        "a stamped restart must not duplicate entries"
    );
}
