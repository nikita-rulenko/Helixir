...
fn test_core_setter_with_default_workers() {
    ...
    assert_eq!(setter.threads_per_core, 2);
    assert_eq!(setter.num_threads(), 4);
}
...
fn test_gateway_opts_default_workers_per_core() {
    assert_eq!(GatewayOpts::DEFAULT_WORKERS_PER_CORE, 2);
}

#[test]
fn test_gateway_opts_accepts_even_worker_override() {
    assert_eq!(
        GatewayOpts::parse_workers_per_core(Some(std::ffi::OsStr::new("4"))),
        Ok(4)
    );
}

#[test]
fn test_gateway_opts_rejects_memory_unsafe_worker_shapes() {
    for value in ["0", "1", "3", "invalid"] {
        assert!(
            GatewayOpts::parse_workers_per_core(Some(std::ffi::OsStr::new(value))).is_err(),
