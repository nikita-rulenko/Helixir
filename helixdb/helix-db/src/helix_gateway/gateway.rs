...
impl GatewayOpts {
    /// Memory-safe default for Helixir's LMDB workload.
    ///
    /// Each read worker owns a complete per-query bump arena. Keeping the
    /// default at two prevents independent full scans from multiplying the
    /// process working set eightfold on a single managed core.
    pub const DEFAULT_WORKERS_PER_CORE: usize = 2;

    /// Resolve the read-worker fan-out from `HELIX_WORKERS_PER_CORE`.
    ///
    /// The worker scheduler requires an even value and at least two workers.
    pub fn workers_per_core_from_env() -> Result<usize, String> {
        Self::parse_workers_per_core(std::env::var_os("HELIX_WORKERS_PER_CORE").as_deref())
    }

    pub(crate) fn parse_workers_per_core(raw: Option<&std::ffi::OsStr>) -> Result<usize, String> {
        let Some(raw) = raw else {
            return Ok(Self::DEFAULT_WORKERS_PER_CORE);
        };
        let value = raw
            .to_str()
            .ok_or_else(|| "HELIX_WORKERS_PER_CORE must be valid UTF-8".to_string())?
            .parse::<usize>()
            .map_err(|_| "HELIX_WORKERS_PER_CORE must be an integer".to_string())?;
        if value < 2 || !value.is_multiple_of(2) {
            return Err("HELIX_WORKERS_PER_CORE must be an even integer >= 2".to_string());
        }
        Ok(value)
    }
}
