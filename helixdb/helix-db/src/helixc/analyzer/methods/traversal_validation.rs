    ...
    for (i, graph_step) in tr.steps.iter().enumerate() {
        ...
        match step {
            ...
            StepType::Update(update) => {
                ...
                // UPDATE preserves cardinality. Collapsing Nodes/Edges to a single
                // value here makes the response projector serialize only the first
                // mutated item even though the storage mutation touched the entire
                // collection.
                gen_traversal.should_collect = ShouldCollect::No;
                excluded.clear();
            }
            ...
            StepType::Range((start, end)) => {
                ...
                // An indexed source starts life as `Type::Node`/`ToObj` because
                // a UNIQUE INDEX lookup is normally scalar. A plain INDEX may
                // legitimately contain duplicate keys, however, and an explicit
                // RANGE is the HQL author's declaration that this traversal is a
                // collection. Keep that information in the generated Rust instead
                // of calling `collect_to_obj()` and rejecting the second match.
                gen_traversal.should_collect = ShouldCollect::ToVec;
                cur_ty = match cur_ty {
...
mod tests {
    ...
    use crate::helixc::generator::traversal_steps::{ShouldCollect, TraversalType};
    ...
    #[test]
    fn indexed_source_with_range_collects_all_duplicate_keys() {
        let source = r#"
            N::Memory { INDEX content_key: String }

            QUERY test(content_key: String) =>
                memories <- N<Memory>({content_key: content_key})::RANGE(0, 100)
                RETURN memories
        "#;

        let content = write_to_temp_file(vec![source]);
        let parsed = HelixParser::parse_source(&content).unwrap();
        let (diagnostics, output) = crate::helixc::analyzer::analyze(&parsed).unwrap();
        assert!(diagnostics.is_empty());

        let traversal = output.queries[0]
            .statements
            .iter()
            .find_map(|statement| match statement {
                GeneratedStatement::Assignment(assignment) => match assignment.value.as_ref() {
                    GeneratedStatement::Traversal(traversal) => Some(traversal),
                    _ => None,
                },
                _ => None,
            })
            .expect("expected indexed traversal assignment");
        assert!(matches!(traversal.should_collect, ShouldCollect::ToVec));
    }

    #[test]
    fn update_over_indexed_collection_preserves_collection_response() {
        let source = r#"
            N::Memory { INDEX content_key: String }

            QUERY test(content_key: String, canonical: String) =>
                memories <- N<Memory>({content_key: content_key})::RANGE(0, 100)
        ...
        let update = output.queries[0]
            .statements
            .iter()
            .find_map(|statement| match statement {
                GeneratedStatement::Assignment(assignment)
                    if assignment.variable.to_string() == "updated" =>
                {
            ...
            .expect("expected update assignment");
        assert!(matches!(
            update.traversal_type,
            TraversalType::Update {
                source_is_plural: true,
                ..
            }
        ));
        assert!(
        ...
        let returned = output.queries[0]
            .return_structs
            .iter()
            .find(|returned| returned.source_variable == "updated")
            .expect("expected updated return projection");
        assert!(returned.is_collection);
    }

    #[test]
    fn update_over_single_indexed_node_preserves_object_response() {
        let source = r#"
            N::Memory { INDEX memory_id: String, content: String }

            QUERY test(memory_id: String, content: String) =>
                memory <- N<Memory>({memory_id: memory_id})::FIRST
                updated <- memory::UPDATE({content: content})
                RETURN updated
        "#;
        ...
        assert!(matches!(
            update.traversal_type,
            TraversalType::Update {
                source_is_plural: false,
                ..
            }
        ));
        assert!(update.to_string().contains("collect_to_obj()?"));
    ...
    // Property errors require a different validation path.
