...
const HELIX_REPO_PATH_ENV: &str = "HELIX_REPO_PATH";
...
pub(crate) async fn ensure_helix_repo_cached() -> Result<()> {
    ...
    let repo_source = resolve_repo_source(
        std::env::var_os(HELIX_REPO_PATH_ENV),
...
fn resolve_repo_source(
    ...
    if let Some(configured_path) = configured_path {
        if configured_path.is_empty() {
            return Err(eyre!("{HELIX_REPO_PATH_ENV} cannot be empty"));
        }
        return Ok(RepoSource::Local(validate_local_repo(Path::new(
            &configured_path,
        ))?));
    }
    ...
    if dev_mode {
        let workspace_root = manifest_dir
            .parent()
            .ok_or_else(|| eyre!("cannot determine the Helix workspace root"))?;
        return Ok(RepoSource::Local(validate_local_repo(workspace_root)?));
    }

    Ok(RepoSource::GitRelease)
}

fn validate_local_repo(path: &Path) -> Result<PathBuf> {
    let canonical = path.canonicalize().map_err(|error| {
        eyre!(
            "{HELIX_REPO_PATH_ENV} points to an unavailable directory {}: {error}",
            path.display()
        )
    })?;
    ...
    for required in [
        "Cargo.toml",
        "helix-db/Cargo.toml",
        "helix-container/Cargo.toml",
    ] {
        let candidate = canonical.join(required);
        if !candidate.is_file() {
            return Err(eyre!(
                "{HELIX_REPO_PATH_ENV}={} is not a HelixDB v2 workspace: missing {required}",
                canonical.display()
            ));
        }
    }
...
fn needs_cache_recreation(repo_cache: &Path, repo_source: &RepoSource) -> Result<bool> {
    ...
    match (repo_source.is_local(), is_git_repo) {
        (true, true) => {
            Step::verbose_substep("Cache is git repo but a local fork is required - recreating...");
            Ok(true)
        }
        (false, false) => {
            Step::verbose_substep(
                "Cache is a local copy but the pinned release repository is required - recreating...",
            );
            Ok(true)
        }
        _ => Ok(false),
...
async fn recreate_helix_cache(repo_cache: &Path, repo_source: &RepoSource) -> Result<()> {
    std::fs::remove_dir_all(repo_cache)?;
    create_helix_cache(repo_cache, repo_source).await
}

async fn create_helix_cache(repo_cache: &Path, repo_source: &RepoSource) -> Result<()> {
    Step::verbose_substep("Caching Helix repository (first time setup)...");

    match repo_source {
        RepoSource::Local(source) => create_local_cache(source, repo_cache)?,
        RepoSource::GitRelease => create_git_cache(repo_cache)?,
    }

    Ok(())
}

async fn update_helix_cache(repo_cache: &Path, repo_source: &RepoSource) -> Result<()> {
    Step::verbose_substep("Updating Helix repository cache...");

    match repo_source {
        RepoSource::Local(source) => update_local_cache(source, repo_cache)?,
        RepoSource::GitRelease => update_git_cache(repo_cache)?,
    }

    Ok(())
}

fn create_local_cache(source: &Path, repo_cache: &Path) -> Result<()> {
    Step::verbose_substep(&format!(
        "Copying maintained local HelixDB workspace from {}...",
        source.display()
    ));
    copy_dir_recursive_excluding(source, repo_cache)
}
...
fn update_local_cache(source: &Path, repo_cache: &Path) -> Result<()> {
...
mod tests {
    ...
    fn create_test_workspace(root: &Path) {
        fs::create_dir_all(root.join("helix-db")).expect("create helix-db directory");
        fs::create_dir_all(root.join("helix-container")).expect("create helix-container directory");
        for relative in [
            "Cargo.toml",
            "helix-db/Cargo.toml",
            "helix-container/Cargo.toml",
    ...
    #[test]
    fn configured_local_repo_overrides_release_mode() {
        let workspace = tempdir().expect("create workspace");
        create_test_workspace(workspace.path());

        let source = resolve_repo_source(
            Some(workspace.path().as_os_str().to_owned()),
            Path::new("/unrelated/manifest"),
            false,
        )
        .expect("resolve configured local repository");
    ...
    #[test]
    fn configured_local_repo_fails_closed_when_workspace_is_incomplete() {
        let workspace = tempdir().expect("create workspace");
        fs::write(workspace.path().join("Cargo.toml"), "[workspace]\n")
            .expect("write root manifest");

        let error = resolve_repo_source(
            Some(workspace.path().as_os_str().to_owned()),
            Path::new("/unrelated/manifest"),
            false,
        )
        ...
        assert!(error.to_string().contains("helix-db/Cargo.toml"));
    }

    #[test]
    fn local_repo_recreates_a_git_cache() {
        let cache = tempdir().expect("create cache");
        fs::create_dir(cache.path().join(".git")).expect("create git marker");
        let source = RepoSource::Local(PathBuf::from("/maintained/fork"));

        assert!(needs_cache_recreation(cache.path(), &source).expect("inspect cache"));
    }
}
