...
- RBAC is graph-backed in HelixDB and is the single source of truth for Rust, MCP, and CLI. New onboarding enables it through the reserved `onboarding` enrollment/compatibility group; `--legacy-trusted-mode` is the explicit disabled escape hatch. Existing users migrate as workers, selected trusted clients as group admins, and only the operator receives global admin. Enabled authorization is deny-by-default and fail-closed. Enrolled onboarding writers may omit `group_id`; other writes require a concrete group. `actor_id` is the authenticated principal and `user_id` is the memory owner/target. Use `helixir rbac`; do not introduce local ACLs or a second user registry.
